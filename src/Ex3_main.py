...
def check2():
    ...
    g_algo.load_from_json(file)
    dist, path = g_algo.shortest_path(80, 21)
    print(dist, path)
    short_path_time = time.time() - start_time
    start_time = time.time()
    print(g_algo.connected_component(4))
    connected_component_time = time.time()-start_time
    start_time = time.time()
    print(g_algo.connected_components())
    connected_components_time = time.time()-start_time
    print("shortest path time" +short_path_time.__str__()+ "connected component time" +connected_component_time.__str__() +\
           "connected components time" + connected_components_time.__str__())
    #g_algo.plot_graph()
